...
### D-116 — Button audit: brand-color recolor, dead-code cleanup, duplicate merge, primary/secondary token normalization
**Status:** Decided · **Binding scope:** Intended behavior + bugfix · **Scope:** Global (ds + c3_c1) · **Triggered by:** A dedicated full-prototype button audit, user direction on scope/color · **Date:** 2026-09-24
**Decision:** A background audit cataloged every non-icon-only button in `c3_c1/index.html` and compared it against `.acuity-button`. Two headline findings: (1) `--acuity-color-dawn-purple-100` (the color actually used pervasively across the wizard's primary CTAs, 41 occurrences) conflicts with the DS's `.acuity-button--primary` fill, which used `--acuity-bg-brand-strong` → Evening-Dark-Blue (`#253848`) instead; (2) several bespoke primary/secondary button classes had drifted off the spacing/type token scale (raw `20px`/`6px`/`400` instead of `--acuity-spacing-*`/`--acuity-font-weight-book`). User chose to recolor the DS to match the prototype's existing purple (not the other way around) and approved 4 scope items: dead-code cleanup, merging exact duplicates, primary CTA consolidation, secondary/outline consolidation.
1. **Brand-color recolor.** `ds/tokens.css`: `--acuity-bg-brand-strong` changed from raw `#253848` to `var(--acuity-color-dawn-purple-100)`. `ds/components.css`: every `--acuity-color-evening-dark-blue-{100,90,80}` reference replaced with the equivalent `--acuity-color-dawn-purple-{100,90,80}` (12 occurrences across `.acuity-button--primary/--secondary/--tertiary`, `.acuity-icon-button--primary/--secondary`, `.acuity-stepper__step--complete`) — done as a full family swap, not just the one `--bg-brand-strong` token, since leaving Secondary/Tertiary/the stepper's "complete" dot on the old blue while Primary went purple would have created a new, more visible internal inconsistency within those same components. Deliberately did **not** touch `--acuity-fg-brand`/`--acuity-border-brand`/`--acuity-bg-brand-subtle`/`--acuity-bg-brand-default` — those drive focus rings, `.acuity-chip--brand`, `.acuity-radio-card`, `.acuity-appointment-card--ai`, and the AI-badge gradient, none of which were part of the button audit or this decision; recoloring those would be a much wider "is our whole brand blue-or-purple" call this session didn't make.
2. **Dead code removed:** `.back-link` (0 usages) and a duplicated `.avail-link` block (0 usages; a verification pass against git history found only 2 copies in the last commit, but 3 removal edits were needed against this session's live, uncommitted file state — likely a 3rd copy introduced by earlier uncommitted work this same session rather than a miscount; net effect is the same, 0 copies remain). **`.chip--plain--small` was deliberately kept** despite 0 usages — its own comment explicitly says "kept as a variant for future use elsewhere," a documented intentional decision, not accidental cruft, so it wasn't swept up in the cleanup.
   **Near-miss, caught and fixed:** `.chip` (base class + `.chip.chip--selected` + its "Interactive transitions" `transition` entry) was initially deleted too, on the same "0 static markup usages" basis — but `.chip` is actually applied dynamically via JS (`_appendS3Chips()`'s `btn.className = 'chip'`, for S2's "Popular searches" dropdown results, with `selectS3Chip()` toggling `chip--selected` on the same elements). A static-markup-only grep can't see this. Caught before finishing this pass and fully restored (verbatim). Flagging the lesson: dynamic `className`/`classList` assignments in JS are invisible to markup-only dead-code searches, and any future "0 usages" claim needs an explicit JS-string-literal grep too, not just a `class="..."` grep.
3. **Exact-duplicate merge:** `.home-plan-cta` (4 uses) and `.home-book-btn` (1 use) — audited as "byte-identical" but actually differed in layout-only properties (inline-fit-content w/ `margin-top`/`align-self` vs. fill-width w/ `width:100%`/`text-align`/`box-sizing`); the *visual recipe* (background/border/font/uppercase/letter-spacing) was genuinely identical. Merged into a new shared `.home-outline-cta` base + two thin layout modifiers (`--inline`, `--full-width`), not a lossy single-class merge.
4. **Primary CTA + secondary/outline token normalization** (kept as local classes matching DS values, same technique as D-093/D-100 rather than swapping markup to literal `.acuity-button` classes — chosen specifically because `.btn-primary`/`.btn-next`'s disabled states are driven by bespoke `classList.toggle('btn-primary--disabled', ...)`/`btn-next--disabled` JS across several call sites, and `.acuity-button` expects native `:disabled`/`aria-disabled` instead; swapping classes without also migrating that JS risked a real functional regression for low payoff): `.btn-primary`, `.btn-prev`/`.btn-next` (shared rule), `.btn-sheet-save`, `.btn-ghost`, `.btn-sheet-close` all had their padding moved onto the `--acuity-spacing-*` scale (previously raw `20px`/`12px 20px`/`6px`/`8px 4px`) and `font-weight: 400` replaced with `var(--acuity-font-weight-book)`. `.btn-sheet-save`'s `color: white` also replaced with `var(--acuity-color-neutral-gray-base)`. `.btn-prev`/`.btn-next`'s `flex:1;max-width:160px;min-width:56px` layout constraint (a real local need — Figma's dual-button "Button Special" nav, no `.acuity-button` equivalent) was left untouched. `.s5-back-btn` was found to already be fully token-based and needed no changes.
**Rationale:** Direct user request following a dedicated audit; color-direction and scope resolved via 2 clarifying questions given the material visual/blast-radius implications.
**Constraints:** None.
**Implementation discretion:** Chose the "keep local class, match DS token values" technique (not a literal `.acuity-button` class swap) for the primary/secondary consolidation specifically because of the disabled-state JS coupling discovered mid-task — flagged as a deliberate scope-narrowing, not an oversight; a full migration to `.acuity-button` classes (with matching JS updates to use `:disabled`/`aria-disabled`) remains a valid, larger follow-up if wanted.
**Artifacts affected:** [ ] MD [ ] Figma [x] Prototype [ ] Ticket
**Supersedes:** The Evening-Dark-Blue brand-color tokens in `.acuity-button`/`.acuity-icon-button`/`.acuity-stepper`; `.home-plan-cta`/`.home-book-btn`; the raw pixel/weight values in the 5 normalized button classes.
**Prototype ref:** `ds/tokens.css`, `ds/components.css`, `c3_c1/index.html`.
**Open questions:** Whether to eventually do the full `.acuity-button`-class migration (including the disabled-state JS rework) for `.btn-primary`/`.btn-next`/`.btn-prev`/`.btn-sheet-save`/`.btn-ghost`/`.btn-sheet-close` — deferred, not blocking. Whether `--acuity-fg-brand`/`--acuity-border-brand`/`--acuity-bg-brand-subtle`/`--acuity-bg-brand-default` should also move to a purple family for full brand coherence (chips, radio cards, focus rings, AI badge/card) — explicitly out of scope for this pass, flagged for a separate decision.
